...
\begin{definition}[Dynamical primitives]
	Suppose the existence of $\mathcal{W}$ as the world-finite structural foundation on condition. There, we define a space called the \textbf{dynamical system} that encapsulate the following. Define the world $\mathsf{W}[\mathcal{W},\mathbb{N}]$ on encapsulation the finite-state system transition. Then, the following primitives are classified as needed. 
	{\small
		\begin{enumerate}[itemsep=1pt,topsep=1pt]
			\item \textbf{Instantiation}: To construct the system, we perform the primitive action of instantiation. From the representation of $\mathrm{ENC}$, and details from $\mathcal{A}$ or with addition of $\mathcal{G}_{\mathbb{N}}$ in variational setting, derive a \textbf{mass containers/space} of which to house instantiation of separated subject matter under such scheme. Second, we posit the existence of a \textbf{instantiation container/space}, of which to contain all instantiated frame operation. Under variational encoding, denoted as $\mathbf{I}$ for the instantiation space, and $\mathbb{M}$ for mass space, then $\mathbb{I}\equiv \mathbb{M}$. Under default scheme, or with reduce system, then $|\mathbb{I}|\leq |\mathbb{M}|$. 
			\item \textbf{}
		\end{enumerate}
	}
\end{definition}

...
Lastly, let us talk about the \textbf{prepositions} that would be contingent before the notion of uncertainty as said to be given, in case something have been forgotten of mentioning. First, the action of assignment, assign from the assignee, requires \textbf{the system to assign and quantify}. This will be conducted not singlehandedly because it would be trivially false in doing so, but we will inherit the encoding \textit{quantification framework} from normal mathematics, which includes discrete quantification, and ultra-discrete (continuous under realistic limit, non-idealism to infinity) quantification scheme of correctness and density. There is the strain between the language of continuity infinite density, i.e. the shift from $1,2,3,4$ to be supplanted by $\int_{[a,b]}pdP$ in the concurrent formalism, and such is unfortunately not resolved by only hand-waived, much to the insufficiency in describing large, dense but finite system of observations, and is indeed the \textit{encoding artefact} of difficult in its form. Second, is that we require a formal construction for the \textbf{frame systems} and \textbf{state system}, since they are the two most system-level components that would help in describe and derive later formalism on itself. Following that, we would be able to have a sufficient account on how \textit{inference} happens, in the kinematic sense without specifying interaction but only consequences\footnote{The force-level interactive inference are often \textit{context-based}, and any amount of generalized notion for such would rather do a disservice to the recipient of the framework. Such would be where can we say as probabilitas e condicionibus, or to say, probabilitas contextualis - the probability under context.} and thus the many interaction of idealized observations in itself. The concrete definition and formulation of what to be \textbf{known} and \textbf{unknown} by then, of $K,U$ to be specified directly and under different frame, how to be unknown and the conditions thereof, would also require such to be defined concretely. Lastly, we are to mention the final clause, of the proposition for uncertainty to exist, is that the \textbf{subject of interest} exists, alongside the presupposition of the ability to \textit{infer}, the ability to \textit{interact}, the ability to create states and the ability for its description to be not \textit{frozen in time}, all of such are presupposed to exist. Hence, objectively speaking, probability is again, something to put on, while existence conditions are there for us to recontextualize what they are. At last, let us state what there is in a principle
\begin{principle}[The assumed existence thereof]
    Of the lineage of thought barring \textbf{universimorphism} and subsequent derivation, let us inquire the following belief:
    \begin{enumerate}[itemsep=1pt,topsep=0.5pt]
        \item \textbf{Subject existence}: let there be of must, something to be uncertain about, and of which something to be certain about, respective of its observer. The objects and space underlying \textbf{must exist}, in one way or another. 
        \item \textbf{Observer existence}: Let there be the observer, and let there be \textbf{perspective}, attached to each. And to let that existence be capable of insinuation in context change. 
        \item \textbf{Inferential capacity}: let there be, such that the observer must be able to form beliefs about the system. 
        \item \textbf{Interactive capacity}: the observer must be able to affect or be affected by it, by intention or of coincidence, of noise or not. 
        \item \textbf{State-creation capacity}: Let the system to be of must, able to take on different configurations, such is to say the existence of \textbf{states} are sufficed, without committing to either camp of determinism or not. 
        \item \textbf{Temporal non-stasis}: Let the system to not be frozen naturally — it must be capable of change, and thus states are performed.
    \end{enumerate}
\end{principle}
That, concludes the probabilistic argument of its presupposition, and the uncertainty-ness that gave rise to this entire conversation. One can ask if there are anything else not supplied? Barring the current understanding, such would be only uncertainty as one of the identification that the system is lacking of - relating to the structure of the system itself and how we interact with it. However, it also includes the \textbf{encoding problem}, of which we spent a chapter or two on. If there are anything, they must not be of the two, leaving the question to be relating to the \textbf{ontology} under uncertainty presupposition, intervention dynamics, and the breaking frame where there exists more than defined here of the framework. But that in itself is uncertainty, and should we do such, let it be the contingency for change of this own foundation. 

...
The bracket is pretty much cosmetic, though, to distinguish from the use of array and matrix, we will use $[\cdot]$ for array and $(\cdot)$ for matrices. We can also see why we choose to write it as such, per a very easy example. In retrospect, it will look like that we extend from vectors, into matrix, 
\begin{equation}
    \lambda\begin{pmatrix}
        x_{1}\\
        x_{2}\\
    ...
    \begin{pmatrix}
        ...
        \end{pmatrix}
\end{equation}
even though such correspondence is often rather more complicated, and it is not really seems to be a direct expansion, aside from entry freedom. That said, the issue would have to be addressed later on, because of said being one of the many discrepancies that gives confusion on the different conventions on how to write objects in an encoding language, in which proliferation, for example of which will be later discussed, \textit{column vector} or \textit{row vector} inside a non-trivial $(m,n)$ matrix full form, are artefact, not like they are inherently a property of the system itself. Then, given such, what can we do with matrices? First, we can \textit{add} them together; given $A,B$ being two matrices, then $A+B$, the \textit{matrix addition} is defined as: 
\begin{equation}
A + B = \begin{pmatrix}
...
\end{equation}
For certain reason in the theory, or \textbf{matrix theory} and linear algebra in general, the matrix multiplication is explicitly defined only for such shape configuration. Otherwise, the matrix multiplication simply does not exist. From what we can see of illustration of matrix multiplication from \cite{ANDRILLI20101}, we have, this bunch:
%\begin{figure}[htb]
%    \centering
...
\end{bmatrix}
}_{m \times p \text{ matrix } C}
\end{equation}
for the array matrix form. Note that here, we can see that for multiplication, it basically \textbf{combines multiple entries'} aggregation together, that is why there exists the formula for $c_{ik}$ in such $a_{ij}b_{jk}$ is summed together. Interestingly, a consequence of that is then the invariance of ordering for any multiplication steps, and thus for comatrix and additional determinant computation, where one has a similar form of operation in such respect. 
\begin{figure}[htb]
    \centering
    \includegraphics[width=0.6\textwidth]{img/matmulss.png}
    \caption{Illustration of matrix multiplication for $n\times n$ shape. The lower case like $2\times 2$ can be applied just the same. Taken from \href{https://siboehm.com/articles/22/Fast-MMM-on-CPU}{Simon Boehm blog page}.}
    \label{fig:matmuls2}
\end{figure}
